 ...
 * 由于类型定义与关系定义属于授权模型聚合内部结构，
 * 即便是读取操作也需要加载完整聚合，而不是走轻量读侧视图。
 *
 * @author kitona
 * @version 1.0.0
 * @since 2025-02-06
...
@Slf4j
@Service
public class AuthorizationModelSchemaApplicationService implements IAuthorizationModelSchemaApplicationService {
    ...
    public boolean addTypeDefinition(AddTypeDefinitionCommand command) {
        ...
        List<RelationDefinition> relationDefs = AuthorizationTypeDefinitionAssembler.toRelationDefinitions(command.getRelations());
        TypeDefinition typeEntity = TypeDefinition.createWithRelations(command.getType(), relationDefs);
    ...
    @Override
    public List<AuthorizationTypeDefinitionResultDTO> listTypeDefinitions(String storeId, String modelId) {
        ...
        List<TypeDefinition> typeDefinitions = modelOpt.get().getTypeDefinitions();
    ...
    @Override
    public AuthorizationTypeDefinitionResultDTO getTypeDefinition(String storeId, String modelId, String type) {
        ...
        Optional<TypeDefinition> typeDefinition = optional.get().getTypeDefinition(type);
        if (typeDefinition.isEmpty()) {
            log.warn("getTypeDefinition 类型定义不存在: storeId={}, modelId={}, type={}", storeId, modelId, type);
            throw new ApplicationException(IError.DATA_NOT_EXIST);
        }

        return AuthorizationTypeDefinitionAssembler.toDTO(typeDefinition.get());
    }
    ...
    public boolean deleteTypeDefinition(String storeId, String modelId, String type) {
        ...
        Optional<TypeDefinition> removed = model.removeTypeDefinition(type);
        if (removed.isEmpty()) {
    ...
    public boolean addRelation(AddRelationCommand command) {
        ...
        AuthorizationModelAggregate model = this.loadForEdit(command.getStoreId(), command.getModelId());
        Optional<TypeDefinition> typeDefinition = model.getTypeDefinition(command.getType());
        if (typeDefinition.isEmpty()) {
    ...
    @Override
    public List<AuthorizationRelationResultDTO> listRelations(String storeId, String modelId, String type) {
        if (StringUtils.isAnyBlank(storeId, modelId, type)) {
        ...
        Optional<TypeDefinition> typeOpt = modelOpt.get().getTypeDefinition(type);
        if (typeOpt.isEmpty()) {
            log.warn("listRelations 类型定义不存在: storeId={}, modelId={}, type={}", storeId, modelId, type);
            return Collections.emptyList();
        }

        TypeDefinition typeEntity = typeOpt.get();
        ...
        return AuthorizationTypeDefinitionAssembler.toRelationDTOList(typeEntity.getId(), relations);
    }

    @Override
    public AuthorizationRelationResultDTO getRelation(String storeId, String modelId, String type,
                                                      String relationName) {
        ...
        return AuthorizationTypeDefinitionAssembler.toRelationDTO(typeOpt.get().getId(), relationDef);
    }
