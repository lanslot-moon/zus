...
import org.kitona.zus.service.application.coordinator.TupleWriteOrchestrator;
import org.kitona.zus.service.application.coordinator.TupleWriteResult;
 ...
 *
 * <p>
 * 提供元组写入用例的应用层实现，负责参数校验、调用协调服务并发布应用事件。
 *
 * <h3>边界说明</h3>
 * <p>
 * {@link RelationTupleEntity} 是独立聚合根；
...
public class WriteApplicationService implements IWriteApplicationService {
    ...
    @Resource
    private TupleWriteOrchestrator tupleWriteOrchestrator;
    ...
    public void write(String storeId, List<WriteTupleCommand> writeTuple) {
        ...
        TupleWriteResult result = tupleWriteOrchestrator.write(storeId, writeTuple);
        if (!result.hasWrites()) {
            return;
        }
        log.debug("WriteApplicationService.write 写入变更日志(WRITE): storeId={}, count={}", storeId, result.writtenKeys().size());
        eventPublisher.publishEvent(new TupleWrittenApplicationEvent(storeId, result.writtenKeys(), result.zookie()));
    }
    ...
    public void delete(String storeId, List<WriteTupleCommand> deleteTuple) {
        ...
        TupleWriteResult result = tupleWriteOrchestrator.delete(storeId, deleteTuple);
        if (!result.hasDeletes()) {
            return;
        }
        log.debug("WriteApplicationService.delete 写入变更日志(DELETE): storeId={}, count={}", storeId, result.deletedKeys().size());
        eventPublisher.publishEvent(new TupleDeletedApplicationEvent(storeId, result.deletedKeys(), result.zookie()));
    }
}
