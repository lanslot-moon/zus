 ...
 * 用户信息 Assembler
 *
 * <p>负责 {@link UserInfoEntity} 与 {@link UserInfoDTO} 之间的转换。
...
public final class UserInfoAssembler {
     ...
     * @return 用户信息 DTO，entity 为 null 时返回 null
     */
    public static UserInfoDTO toDTO(UserInfoEntity entity, String userAddress) {
        if (entity == null) {
            return null;
        }
     ...
     */
    public static List<UserInfoDTO> toDTOList(List<UserInfoEntity> entities) {
        if (entities == null || entities.isEmpty()) {
            return Collections.emptyList();
        }
        return entities.stream()
                .map(UserInfoAssembler::toDTO)
