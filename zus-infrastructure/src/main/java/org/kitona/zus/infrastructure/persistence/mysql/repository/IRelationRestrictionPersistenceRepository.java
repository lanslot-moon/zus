...
/**
 * 关系类型限制持久化仓储接口（基础设施层，仅 fga_relation_restriction 单表）
 *
 ...
 * @since 2025-02-06
 */
public interface IRelationRestrictionPersistenceRepository extends IService<RelationRestrictionPO> {

    /**
     * 按关系定义 ID 查询该关系下所有类型限制，未删除，按 id 升序
     *
     * @param relationDefinitionId 关系定义主键 ID（fga_model_relation.id）
     * @return 关系类型限制列表，无则返回空列表
     */
    List<RelationRestrictionPO> selectByRelationDefinitionId(Long relationDefinitionId);

    /**
     * 根据关系定义ID集合查询关系限制列表
     *
     * @param relationDefinitionIdSet 关系定义ID集合，用于筛选特定关系定义的限制条件
     * @return 返回符合条件的关系限制对象列表(List<RelationRestrictionPO>)
     */
    List<RelationRestrictionPO> selectByRelationDefinitionId(Set<Long> relationDefinitionIdSet);
